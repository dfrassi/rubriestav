...
class userHelperToolbar extends JObject

{
	...
	function custom($task = '', $icon = '', $iconOver = '', $alt = '', $listSelect = true, $x = false) {
		...
		$dhtml = "<a href=\"#\" onclick=\"javascript:hideMainMenu();submitbutton('$task');\" >" . "<img src=\"$icon\" title=\"$title\" alt=\"my image\">" . "</a><br>$title";

		$bar->appendButton ( 'Custom', $dhtml, $task );
	}
	...
	function customrubriestav($task = '', $alt = '', $cssClass = '', $link='') {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$title = JText::_ ( $alt );
		if($link==""){
			$dhtml = "<a class=\"toolbar\" onclick=\"javascript:hideMainMenu(); submitbutton('".$task."')\" href=\"#\">" .
		"<span  class=\"".$cssClass."\" title=\"".$title."\"></span>".$title."</a>";
		}else{
			$dhtml = "<a class=\"toolbar\" href=\"$link\">" .
		"<span  class=\"".$cssClass."\" title=\"".$title."\"></span>".$title."</a>";
				
		}

		//echo("dhtml = ".$dhtml.$cssClass);
	...
	function approve($task = 'approve', $alt = 'Approve') {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		// Add a publish button
		//$bar->appendButton( 'Publish', false, $alt, $task );
		$bar->appendButton ( 'Standard', 'approve', $alt, $task, false, false );
	}
	/**
	...
	function approveList($task = 'approve', $alt = 'Approve') {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		// Add a publish button (list)
		$bar->appendButton ( 'Standard', 'publish', $alt, $task, true, false );
	}
	...
	function excel() {
		...
		// Add an upload button
		//$bar->appendButton ( 'Popup', 'upload', $alt, "index.php?option=com_media&tmpl=component&task=popupUpload&directory=$directory", 640, 520 );
		$bar->appendButton ( 'Link', 'excel', 'Excel', 'index.php?option=com_rubriestav&controller=user&format=xls' );
	}
...
function loader() {
		...
		// Add an upload button
		//$bar->appendButton ( 'Popup', 'upload', $alt, "index.php?option=com_media&tmpl=component&task=popupUpload&directory=$directory", 640, 520 );
		$bar->appendButton ( 'Link', 'backup', 'Backup', 'index.php?option=com_rubriestav&controller=loader' );
	}
	function purge() {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$bar->appendButton ( 'Link', 'purge', 'Purge', 'index.php?option=com_rubriestav&controller=user&task=purge' );
	}
	
	function importusers() {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$bar->appendButton ( 'Link', 'importusers', 'Import JUser', 'index.php?option=com_rubriestav&controller=importusers' );
	}
	
	function home() {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$bar->appendButton ( 'Link', 'home', 'Home', 'index.php?option=com_rubriestav&controller=user' );
	}
}
