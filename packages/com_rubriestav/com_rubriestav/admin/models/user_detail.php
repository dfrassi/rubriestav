...
class user_detailModeluser_detail extends JModel {
	...
	function _loadData() {
		...
		if ($rubriestav != "1") {
			if (empty ( $this->_data )) {
				$query = 'SELECT h.* FROM ' . $this->_table_prefix . 'user AS h' . ' WHERE h.id = ' . $this->_id;
			...
			if (empty ( $this->_data )) {
				...
				FROM #__rubriestav_user AS h
				left join #__rubriestav_tb_utenti as u on (u.id = h.idjusers)
	...
	function store($data) {
		...
		/*if ($juser->block == 1 && $juser->activation != "") {
			$row->status = 0; // 0 � In attesa di conferma
		}
		if ($juser->block == 0) {
			$row->status = 1; // 1 � Confermato 
		}
		if ($juser->block == 1 && $juser->activation == "") {
			$row->status = 2; // 2 � Bloccato 
		}*/
	...
	function getrubriestavUserFromJuser($id) {
		global $mainframe;
		$query = 'SELECT * FROM #__rubriestav_user where idjusers = ' . $id;
		$this->_db->setQuery ( $query );
		return $this->_db->loadObjectList ();
	}
