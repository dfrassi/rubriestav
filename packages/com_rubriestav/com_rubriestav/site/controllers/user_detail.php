...
class user_detailController extends JController {
	...
	function saveFile() {
		...
		if (strtolower ( JFile::getExt ( $filename ) ) == 'jpg') {
			if (JFile::upload ( $src, $dest )) {
				...
				$msg = "Impossibile caricare il file - Controllare l'estensione, sono contemplati soltanto immagini con estensione JPG";
	...
	function cancel() {
		...
		$popop = $this->checkUser($cid[0]);
		
		//if ($user->id == $popop)
		//$this->setRedirect ( 'index.php' );
		//else
		$this->setRedirect ( 'index.php?option=com_rubriestav' );
	}
	...
	function checkUser($idJoomlacl){
		
		$queryArrayUtenti = "select idjusers from #__rubriestav_user where id = $idJoomlacl";
		//echo($queryArrayUtenti);
		$arrayUtenti = utility::getArray($queryArrayUtenti);
		return $arrayUtenti[0]->idjusers;
